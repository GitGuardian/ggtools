...
if [[ "$FORCE" == "true" ]] || [[ "$status" == "completed" ]]; then
  ...
  postgres_restore_cmd="PGPASSWORD=\$POSTGRES_POSTGRES_PASSWORD psql -U postgres"
  gunzip -c ${INPUT_FILE} | \
    kubectl $KUBECTL_ARGS \
      exec -i $pod -- \
...
pod=$(kubectl $KUBECTL_ARGS get pod \
  --field-selector="status.phase=Running" \
  --selector="kots.io/app-slug=${REPLICATED_APP}, app.kubernetes.io/component=internal-api" \
  --output=name 2>$ERROR_LOG_FILE | head -1)
if [[ -z "$pod" ]]; then
