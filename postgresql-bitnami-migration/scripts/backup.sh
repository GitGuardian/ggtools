...
postgres_pod_selector="app.kubernetes.io/name=postgresql, app.kubernetes.io/component=primary"
...
BACKUP_DIR=$(mktemp -d)
echo_step "Saving PostgreSQL StatefulSet configuration in ${BACKUP_DIR}"
STATEFULSET_NAME=$(kubectl $KUBECTL_ARGS get statefulset \
  --selector="$postgres_pod_selector" \
  -o jsonpath='{.items[0].metadata.name}')
kubectl $KUBECTL_ARGS get statefulset "$STATEFULSET_NAME" -o yaml \
  > "$BACKUP_DIR/${STATEFULSET_NAME}_backup.yaml" 2>$ERROR_LOG_FILE
if [[ -s "$ERROR_LOG_FILE" ]]; then
  exit 1
fi
echo_ok
