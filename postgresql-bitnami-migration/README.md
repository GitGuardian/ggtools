# PostgreSQL Bitnami migration (KOTS only)

For customers using embedded PostgreSQL database, with GitGuardian version prior to `2026.1.0`, we are moving from PostgreSQL Bitnami chart to our own maintained chart.
...
cd ggtools/postgresql-bitnami-migration/scripts
```
...
The script will perform the following steps:
  ...
  5. Delete the postgresql `StatefulSet` object.

### Update GitGuardian application

From the KOTS admin console you can now update the GitGuardian application.

:vertical_traffic_light: A first preflight check called `PostgreSQL Bitnami deprecation upgrade` will ensure that the backup was executed correctly by checking the configMap `postgresql-backup`.
:vertical_traffic_light: A seconde preflight check called `postgresql Status` will ensure that the previous postgresql `StatefulSet` (if any) is deleted before the upgrade. `StatefulSet` objects are immutable and must be deleted for this specific upgrade.
